...
public class Build {
  ...
  public static String longestWord(Vertex<String> vertex, Set<Vertex<String>> seen) {
    if(vertex == null || seen.contains(vertex)) return "";

    seen.add(vertex);

    String l = vertex.data;

    for(Vertex<String> neighbor : vertex.neighbors) {
      String possibleL = longestWord(neighbor, seen);
      if(possibleL.length() > l.length()) {
        l = possibleL;
      }
    }
  ...
  public static <T> Set<T> unreachable(Map<T, List<T>> graph, T starting) {
    ...
    Set<T> cantReach = new HashSet<>();
    //everything needs to be added
    cantReach.addAll(graph.keySet());
    //stuff we can reach needs to be removed
    cantReach.removeAll(seen);
    return cantReach;
  }
  ...
  private static <T> void unreachable(Map<T, List<T>> graph, T starting, Set<T> seen) {
    ...
    if(graph.get(starting) == null) return;
